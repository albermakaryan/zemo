...
class RecorderPanel(tk.Frame):
    ...
    def _build(self):
        ...
        self._canvas = tk.Canvas(
            self,
            width=config.PREVIEW_W,
            height=config.PREVIEW_H,
            bg="#050505",
    ...
    def _draw_placeholder(self):
        ...
        self._canvas.create_text(
            cx, cy, text="No preview", font=config.MONO_SM, fill=config.MUTED
        )
    ...
    def _start_recording(self, start_barrier=None, email=None):
        ...
        self._btn_record.config(
            text="⏹  Stop", bg=config.BG3, fg=config.RED, activebackground=config.BG3
        )
    ...
    def _on_frame(self, frame, elapsed):
        try:
            ...
            self._canvas.after(
                0,
                lambda: self._canvas.create_image(
