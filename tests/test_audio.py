...
def _verify_audio_wav(path: Path) -> bool:
    ...
    try:
        ...
        print("  Verify: {:.2f} s, {} Hz, {} ch, RMS={:.4f}, peak={:.4f}".format(
...
def run_audio(seconds: float = 10.0) -> int:
    ...
    if sys.platform == "win32":
        ...
        if not is_loopback_available():
            print("FAIL: No internal audio (monitor/loopback). Internal = computer playback only (video, apps), not microphone.")
            if sys.platform == "darwin":
                print("  macOS has no built-in loopback. Install BlackHole or Loopback for internal audio.")
            else:
                print("  Install: pip install -e \".[linux]\"  and ensure PulseAudio/PipeWire exposes a monitor source (e.g. <sink>.monitor).")
            print("  We only use devices that are explicitly monitor/loopback — never 'pulse' or default (usually mic).")
            print("  Available input devices:")
            list_audio_devices()
            return 1
        print("System audio test ({} monitor) — recording {} s...".format("Linux" if sys.platform == "linux" else "macOS", seconds))
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test system audio recording; runs recorder then verifies WAV.")
    parser.add_argument("--seconds", type=float, default=10.0, help="Recording length")
    parser.add_argument("--list-devices", action="store_true", help="List input devices and exit (debug no monitor)")
